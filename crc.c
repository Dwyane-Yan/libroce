...
static const uint32_t roce_crc_table[256] = {
...
#define roce_crc32_1(buf)                                                                          \
    {                                                                                              \
        crc = roce_crc_table[((int)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);                           \
    }
#define roce_crc32_2(buf)                                                                          \
    {                                                                                              \
        roce_crc32_1(buf);                                                                         \
        roce_crc32_1(buf);                                                                         \
    }
#define roce_crc32_4(buf)                                                                          \
    {                                                                                              \
        roce_crc32_2(buf);                                                                         \
        roce_crc32_2(buf);                                                                         \
    }
#define roce_crc32_8(buf)                                                                          \
    {                                                                                              \
        roce_crc32_4(buf);                                                                         \
        roce_crc32_4(buf);                                                                         \
    }

static inline uint32_t __roce_crc32(uint32_t crc, uint8_t *buf, uint32_t len)
{
    while (len >= 8) {
        roce_crc32_8(buf);
        len -= 8;
...
uint32_t roce_crc32(uint32_t crc, struct iovec *iovs, uint32_t num_iov)
{
    ...
    for (uint32_t i = 0; i < num_iov; i++) {
        ...
        crc = __roce_crc32(crc, iov->iov_base, iov->iov_len);
    }
                        ...
                        ib_bth *bth, struct iovec *iovs, uint32_t num_iov)
{
    ...
    if (is_ipv4) {
        ip4 = ip;
        ip4->tos = 0xff;
        ip_hdr_set_ttl(ip4, 0xff);
        ip_hdr_set_checksum(ip4, 0xffff);
        icrc = __roce_crc32(icrc, (uint8_t *)ip4, sizeof(ip_hdr));

        if (ihllen) {
            icrc = __roce_crc32(icrc, (uint8_t *)ihlbuf, ihllen);
        }
    } else {
        ip6 = ip;
        ipv6_hdr_set_version_tc_fl(ip6, 6, 0xff, 0xffffff);
        ipv6_hdr_set_hop_limit(ip6, 0xff);
        icrc = __roce_crc32(icrc, (uint8_t *)ip6, sizeof(ipv6_hdr));
    }

    udph->checksum = 0xffff;
    icrc = __roce_crc32(icrc, (uint8_t *)udph, sizeof(udp_hdr));

    /* exclude reserved fields of bth.u32_1 */
    bth->u32_1 |= htobe32(~IB_BTH_DESTQP_MASK);
    icrc = __roce_crc32(icrc, (uint8_t *)bth, sizeof(ib_bth));

    return ~roce_crc32(icrc, iovs, num_iov);
}
